...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "}\n"
    "\n"
    "lowp float packBools(bool a, bool b, bool c) {\n"
    "  return dot(vec3(float(a), float(b), float(c)), vec3(2.0, 4.0, 8.0)) / 255.0;\n"
    "}\n"
    "\n"
    "lowp float packFloats(lowp vec2 values) {\n"
    "  lowp float result = 0.0;\n"
    "  result += floor(values.x * (255.0 / 16.0)) * 16.0;\n"
    "  result += floor(values.y * (255.0 / 16.0));\n"
    "  return result / 255.0;\n"
    "}\n"
    "\n"
    ...
    "  result.x = (min(distance(c, e), distance(c, w)) / (distance(e, w) + EPSILON)) * step(threshold, distance(w, e));\n"
    "  result.y = (min(distance(c, n), distance(c, s)) / (distance(n, s) + EPSILON)) * step(threshold, distance(n, s));\n"
    "\n"
    ...
    "  gl_FragColor = vec4(\n"
    "    flags.type,\n"
    "    packBools(flags.flip.x, flags.flip.y, flags.flip.z),\n"
...
const std::string ShaderManager::cut2UpscalePass1Vertex =
    ...
    "void main() {\n"
    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001;\n"
//    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.125 + vec2(0.0, 0.0);\n"
    "  screenCoords = coords * textureSize - vec2(0.5);\n"
    "  c05 = (screenCoords + vec2(+0.0, +0.0)) / textureSize;\n"
    "  c06 = (screenCoords + vec2(+1.0, +0.0)) / textureSize;\n"
    "  c09 = (screenCoords + vec2(+0.0, +1.0)) / textureSize;\n"
...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "};\n"
    "\n"
    "bvec3 unpackBools(lowp float values) {\n"
    "  lowp vec3 modulos = mod(values * vec3(0.5, 0.25, 0.125), vec3(2.0));\n"
    "  return greaterThan(floor(modulos), vec3(0.5));\n"
    "}\n"
    "\n"
    "lowp vec2 unpackFloats(lowp float value) {\n"
    "  lowp vec2 result = vec2(0.0);\n"
    "  result.x = floor(value * 255.0 / 16.0) * 16.0 / 255.0;\n"
    "  result.y = (value - result.x) * 255.0 * 16.0 / 255.0;\n"
    ...
    "  bvec3 flagBits = unpackBools(floor(flagsPixel.y * 255.0 + 0.5));\n"
    "  lowp vec2 edgeWeights0 = unpackFloats(flagsPixel.z) * 0.5 - 0.25;\n"
    "  lowp vec2 edgeWeights1 = unpackFloats(flagsPixel.w) * 0.5 - 0.25;\n"
    "\n"
    "  Flags flags;\n"
    ...
    "}\n"
    "\n"
    "lowp vec3 blend(lowp vec3 a, lowp vec3 b, lowp float edge, lowp float t) {\n"
    "  lowp float displacement = edge;\n"
    "  lowp float centre = 0.5 + displacement;\n"
    "  lowp float smoothness = (0.5 - sharpness(luma(a), luma(b))) * (1.0 - abs(2.0 * displacement)) + EPSILON;\n"
    "  lowp float nt = linearStep(centre - smoothness, centre + smoothness, t);\n"
    "  return mix(a, b, nt);\n"
    "}\n"
    "\n"
    "Pattern pattern0(Pixels pixels, lowp vec4 edgeWeights, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  result.pixels = pixels;\n"
    "  result.weights = vec3(pxCoords.x, pxCoords.x, pxCoords.y);\n"
    "  result.edgeWeights = vec3(edgeWeights.x, edgeWeights.z, mix(edgeWeights.w, edgeWeights.y, pxCoords.x));\n"
    "  return result;\n"
    "}\n"
    "\n"
    "Pattern pattern1(Pixels pixels, lowp vec4 edgeWeights, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  if (pxCoords.y > pxCoords.x) {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p2, pixels.p2, pixels.p3);\n"
    ...
    "    result.edgeWeights = vec3(edgeWeights.w, edgeWeights.z, 0.0);\n"
    "  } else {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p1, pixels.p1, pixels.p3);\n"
    "    result.weights.x = pxCoords.x - pxCoords.y;\n"
    "    result.weights.y = 1.0 - result.weights.x;\n"
    "    result.weights.z = pxCoords.y / (result.weights.y + EPSILON);\n"
    "    result.edgeWeights = vec3(edgeWeights.x, edgeWeights.y, 0.0);\n"
    "  }\n"
    "  return result;\n"
    "}\n"
    "\n"
    "Pattern pattern2(Pixels pixels, lowp vec4 edgeWeights, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    ...
    "    result.weights.z = pxCoords.x / (result.weights.y + EPSILON);\n"
    "    result.edgeWeights = vec3(edgeWeights.w, edgeWeights.z, mix(edgeWeights.w, edgeWeights.y, pxCoords.x));\n"
    "  } else {\n"
    ...
    "}\n"
    "\n"
    "Pattern pattern3(Pixels pixels, lowp vec4 edgeWeights, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  if (pxCoords.y > 2.0 * pxCoords.x) {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p2, pixels.p2, pixels.p3);\n"
    "    result.weights.x = pxCoords.y - 2.0 * pxCoords.x;\n"
    ...
    "    result.weights.z = pxCoords.y / (result.weights.y + EPSILON);\n"
    "    result.edgeWeights = vec3(edgeWeights.x, edgeWeights.y, mix(edgeWeights.w, edgeWeights.y, pxCoords.x));\n"
    "  } else {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p1, pixels.p2, pixels.p3);\n"
    "    result.weights.x = pxCoords.x - 0.5 * pxCoords.y;\n"
    ...
    "  return result;\n"
    "}\n"
    "Pattern pattern4(Pixels pixels, lowp vec4 edgeWeights, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  result.pixels = pixels;\n"
    ...
    "  result.weights.z = pxCoords.y;\n"
    "  result.edgeWeights = vec3(edgeWeights.x, edgeWeights.z, mix(edgeWeights.w, edgeWeights.y, result.weights.x));\n"
    "  return result;\n"
    "}\n"
    "\n"
    "\n"
    ...
    "\n"
    "  if (flags.type == 0) {\n"
    "    pattern = pattern0(pixels, flags.edgeWeight, pxCoords);\n"
    "  } else if (flags.type == 1) {\n"
    "    pattern = pattern1(pixels, flags.edgeWeight, pxCoords);\n"
    "  } else if (flags.type == 2) {\n"
    "    pattern = pattern2(pixels, flags.edgeWeight, pxCoords);\n"
    "  } else if (flags.type == 3) {\n"
    "    pattern = pattern3(pixels, flags.edgeWeight, pxCoords);\n"
    "  } else {\n"
    "    pattern = pattern4(pixels, flags.edgeWeight, pxCoords);\n"
    "  }\n"
    "\n"
    "  lowp vec3 final = blend(\n"
