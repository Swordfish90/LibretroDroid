...
const std::string ShaderManager::cut3UpscalePass0Fragment =
    ...
    "  lowp float diff1 = abs(a - d);\n"
    "  lowp float diff2 = max(min(abs(a - b), abs(d - b)), min(abs(a - c), abs(d - c)));\n"
    ...
    "  // Saddle fix\n"
    "  if (d05_10 && d06_09) {\n"
    "    lowp float diff1 = abs(l06 - l01) + abs(l11 - l06) + abs(l09 - l04) + abs(l14 - l09);\n"
    "    lowp float diff2 = abs(l05 - l02) + abs(l08 - l05) + abs(l10 - l07) + abs(l13 - l10);\n"
    "    d05_10 = max(diff1, EDGE_MIN_CONTRAST) < diff2;\n"
    "    d06_09 = max(diff2, EDGE_MIN_CONTRAST) < diff1;\n"
    "  }\n"
    "\n"
    "  // Vertical diagonals\n"
    ...
    "\n"
    "  bool lowerCut = any(bvec4(all(d05_11), all(d07_09), all(d05_14), all(d06_13)));\n"
    "  bool upperCut = any(bvec4(all(d01_10), all(d02_09), all(d04_10), all(d06_08)));\n"
    "\n"
    ...
    "    final.z = pack(lowerCut, upperCut, false);\n"
    ...
    "    final.z = pack(lowerCut || upperCut, false, false);\n"
...
const std::string ShaderManager::cut3UpscalePass1Fragment =
    ...
    "}\n"
    "\n"
    "lowp vec3 pattern0(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  cs = mat4(cs[0], cs[1], cs[2], cs[3]);\n"
    "  return vec3(0.0, pxCoords.x, pxCoords.y);\n"
    "}\n"
    "lowp vec3 pattern1(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  lowp vec3 result;\n"
    ...
    "}\n"
    "\n"
    "lowp vec3 pattern2(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  lowp vec3 result;\n"
    ...
    "}\n"
    "\n"
    "lowp vec3 pattern3(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  lowp vec3 result;\n"
    "  if (pxCoords.y > 2.0 * pxCoords.x) {\n"
    "    cs = mat4(cs[0], cs[2], cs[2], cd);\n"
    "    result = vec3(1.0, pxCoords.x * 2.0, pxCoords.y);\n"
    ...
    "}\n"
    "\n"
    "lowp vec3 pattern4(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  lowp vec3 result;\n"
    "  if (pxCoords.x < 0.5) {\n"
    ...
    "}\n"
    "\n"
    "lowp vec3 pattern5(inout lowp mat4 cs, lowp vec4 ab, lowp vec4 cd, lowp vec2 pxCoords) {\n"
    "  lowp vec3 result;\n"
    ...
    "  }\n"
    "\n"
    "  lowp vec4 ab = patternFlags.y > 0.5 ? colors[0] : colors[1];\n"
    "  lowp vec4 cd = patternFlags.x > 0.5 ? colors[2] : colors[3];\n"
    "\n"
    "  lowp vec3 result;\n"
    "\n"
    "  if (pattern == 0) {\n"
    "    result = pattern0(colors, ab, cd, pxCoords);\n"
    "  } else if (pattern == 1) {\n"
    "    result = pattern1(colors, ab, cd, pxCoords);\n"
    "  } else if (pattern == 2) {\n"
    "    result = pattern2(colors, ab, cd, pxCoords);\n"
    "  } else if (pattern == 3) {\n"
    "    result = pattern3(colors, ab, cd, pxCoords);\n"
    "  } else if (pattern == 4) {\n"
    "    result = pattern4(colors, ab, cd, pxCoords);\n"
    "  } else {\n"
    "    result = pattern5(colors, ab, cd, pxCoords);\n"
    "  }\n"
    "\n"
    "  lowp vec3 weights = result.x > 0.5 ? triangle(result.yz) : quad(result.yz);\n"
    "\n"
    "  lowp vec3 final = blend(\n"
    "    blend(colors[0].rgb, colors[1].rgb, weights.x),\n"
    "    blend(colors[2].rgb, colors[3].rgb, weights.y),\n"
    "    weights.z\n"
    "  );\n"
    "\n"
    "#if SPLIT_DEMO_VIEW\n"
