...
const std::string ShaderManager::cutUpscaleVertex =
    ...
    "\n"
    "void main() {\n"
//    "  coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.000001 * 0.1;\n"
    "  coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.000001;\n"
...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "  lowp float dx = sharpness * 0.5;\n"
//    "  lowp float dx = dist * 0.5;\n"
    ...
    "    xdw = td.xy;\n"
    "  }\n"
    "  lowp float wu = sharpSmooth(xuw.x, xuw.y, clamp(xu, 0.0, 1.0), sharpness);\n"
    "  lowp float wd = sharpSmooth(xdw.x, xdw.y, clamp(xd, 0.0, 1.0), sharpness);\n"
//    "  lowp float wy = p.y;\n"
    "  lowp float wy = sharpSmooth(0.0, 1.0, p.y, sharpness);\n"
    "  return mix(mix(a, b, wu), mix(c, d, wd), wy);\n"
    "}\n"
    "\n"
    "lowp float round(lowp float value, lowp float levels) {\n"
    "  return sign(value) * floor(abs(value) * levels + 0.5) / levels;\n"
    "}\n"
    "\n"
    "void main() {\n"
...
//    "  lowp vec3 t4 = texture2D(texture, c4).rgb;\n"
    ...
    "\n"
    "\n"
    "  lowp vec2 edgeDirection = vec2(l1 - l2 + l4 - l3, l1 + l2 - l4 - l3);\n"
...
//    "  lowp float contrast = 0.5 * (absEdgeDirection.x + absEdgeDirection.y);\n"
...
//    "  lowp vec2 pxCoords = fract(screenCoords);\n"
    "\n"
    "  bool hasAngle = abs(edgeDirection.x) + abs(edgeDirection.y) >= 0.05;\n"
    "  bool invertAngle = hasAngle && absEdgeDirection.y > absEdgeDirection.x;\n"
    "  lowp float tana = invertAngle ? edgeDirection.x / edgeDirection.y : edgeDirection.y / edgeDirection.x;\n"
    "\n"
    ...
    "    invertAngle ? t2 : t4,\n"
    "    t3,\n"
    "    hasAngle ? round(tana, LEVELS) : 0.0,\n"
