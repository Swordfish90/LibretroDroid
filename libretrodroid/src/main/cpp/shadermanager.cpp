...
const std::string ShaderManager::diamondUpscaleFragment =
    ...
    "}\n"
    "lowp float sharpSmooth(lowp float t, lowp float smoothness) {"
    "  return smoothstep(0.5 - smoothness, 0.5 + smoothness, t);"
    "}\n"
    "lowp vec3 quadBilinear(lowp vec3 a, lowp vec3 b, lowp vec3 c, lowp vec3 d, lowp vec2 p, lowp float smoothness) {\n"
    "  lowp float x = sharpSmooth(p.x, smoothness);\n"
    "  lowp float y = sharpSmooth(p.y, smoothness);\n"
    "  return mix(mix(a, b, x), mix(c, d, x), y);\n"
    "}\n"
    "// Fast computation of barycentric coordinates only in the sub-triangle 1 2 4\n"
    "lowp vec3 fastBarycentric(lowp vec2 p, lowp float smoothness) {\n"
    "  lowp float l0 = sharpSmooth(1.0 - p.x - p.y, smoothness);\n"
    "  lowp float l1 = sharpSmooth(p.x, smoothness);\n"
    "  return vec3(l0, l1, 1.0 - l0 - l1);\n"
    "}\n"
    "\n"
    "lowp vec3 triangleInterpolate(lowp vec3 t1, lowp vec3 t2, lowp vec3 t3, lowp vec3 t4, lowp vec2 c, lowp float smoothness) {\n"
    ...
    "  lowp vec3 weights = fastBarycentric(triangleCoords, smoothness);\n"
    ...
    "  lowp float contrast = max(max(l1, l2), max(l3, l4)) - min(min(l1, l2), min(l3, l4));\n"
    "  contrast = smoothstep(DYNAMIC_SMOOTHING_MIN_CONTRAST, DYNAMIC_SMOOTHING_MAX_CONTRAST, contrast);\n"
    "  lowp float smoothness = mix(DYNAMIC_SMOOTHING_MAX, DYNAMIC_SMOOTHING_MIN, contrast);\n"
    "#else\n"
    "  const lowp float smoothness = STATIC_SMOOTHING;\n"
    "#endif\n"
    "\n"
    ...
    "\n"
    "  lowp vec3 final = quadBilinear(t1, t2, t4, t3, pxCoords, smoothness);\n"
...
ShaderManager::Data ShaderManager::getShader(Type type) {
    ...
    case Type::SHADER_DIAMOND_UPSCALE_SHARP:
        return {
            ...
            "#define USE_DYNAMIC_SMOOTHING 1\n"
            "#define DYNAMIC_SMOOTHING_MIN_CONTRAST 0.20\n"
            "#define DYNAMIC_SMOOTHING_MAX_CONTRAST 0.80\n"
            "#define DYNAMIC_SMOOTHING_MIN 0.20\n"
            "#define DYNAMIC_SMOOTHING_MAX 0.40\n"
    ...
    case Type::SHADER_DIAMOND_UPSCALE_SMOOTH:
        return {
            ...
            "#define DYNAMIC_SMOOTHING_MIN_CONTRAST 0.20\n"
            "#define DYNAMIC_SMOOTHING_MAX_CONTRAST 0.80\n"
            "#define DYNAMIC_SMOOTHING_MIN 0.25\n"
            "#define DYNAMIC_SMOOTHING_MAX 0.50\n"
            + diamondUpscaleFragment ,
            false
        };
    }
