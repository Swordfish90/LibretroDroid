...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "#define N 4\n"
            "\n"
            "HIGHP vec2 walkN(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  HIGHP vec2 result = vec2(0.0, 0.5);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            ...
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += 0.5; }\n"
            "    else if (currentPattern == 1) { result.x += 1.0; }\n"
            "    if (currentPattern != 1) { break; }\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "HIGHP vec2 walkS(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  HIGHP vec2 result = vec2(0.0, 0.5);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(0.0, +dc.y * float(i));\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            ...
            "  return result;\n"
            "}\n"
            "HIGHP vec2 walkE(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  HIGHP vec2 result = vec2(0.0, 0.5);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            ...
            "    else if (currentPattern == 4) { result.y = 0.0; }\n"
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += 0.5; }\n"
            "    else if (currentPattern == 2) { result.x += 1.0; }\n"
            "    if (currentPattern != 2) { break; }\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "HIGHP vec2 walkW(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  HIGHP vec2 result = vec2(0.0, 0.5);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(-dc.x * float(i), 0.0);\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            ...
            "  if (pattern == 0) {\n"
            "  } else if (pattern == 1) {\n"
            "    HIGHP vec2 resultN = walkN(previousPass, passCoords);\n"
            "    HIGHP vec2 resultS = walkS(previousPass, passCoords);\n"
            "    edges.x = blend(resultN.y, resultS.y, (0.0 + resultN.x) / (1.0 + resultS.x + resultN.x));\n"
            "    edges.z = blend(resultN.y, resultS.y, (1.0 + resultN.x) / (1.0 + resultS.x + resultN.x));\n"
            "  } else if (pattern == 2) {\n"
            "    HIGHP vec2 resultW = walkW(previousPass, passCoords);\n"
            "    HIGHP vec2 resultE = walkE(previousPass, passCoords);\n"
            "    edges.w = blend(resultW.y, resultE.y, (0.0 + resultW.x) / (1.0 + resultW.x + resultE.x));\n"
            "    edges.y = blend(resultW.y, resultE.y, (1.0 + resultW.x) / (1.0 + resultW.x + resultE.x));\n"
            "  } else if (pattern == 3) {\n"
            "    HIGHP vec2 resultN = walkN(previousPass, passCoords);\n"
            "    HIGHP vec2 resultS = walkS(previousPass, passCoords);\n"
            "    edges.x = blend(1.0, resultN.y, 0.5 / (resultN.x + 0.5));\n"
            "    edges.z = blend(0.0, resultS.y, 0.5 / (resultS.x + 0.5));\n"
            "\n"
            "    HIGHP vec2 resultW = walkW(previousPass, passCoords);\n"
            "    HIGHP vec2 resultE = walkE(previousPass, passCoords);\n"
            "    edges.w = blend(1.0, resultW.y, 0.5 / (resultW.x + 0.5));\n"
            "    edges.y = blend(0.0, resultE.y, 0.5 / (resultE.x + 0.5));\n"
            "  } else if (pattern == 4) {\n"
            "    HIGHP vec2 resultN = walkN(previousPass, passCoords);\n"
            "    HIGHP vec2 resultS = walkS(previousPass, passCoords);\n"
            "    edges.x = blend(0.0, resultN.y, 0.5 / (resultN.x + 0.5));\n"
            "    edges.z = blend(1.0, resultS.y, 0.5 / (resultS.x + 0.5));\n"
            "\n"
            "    HIGHP vec2 resultW = walkW(previousPass, passCoords);\n"
            "    HIGHP vec2 resultE = walkE(previousPass, passCoords);\n"
            "    edges.w = blend(0.0, resultW.y, 0.5 / (resultW.x + 0.5));\n"
            "    edges.y = blend(1.0, resultE.y, 0.5 / (resultE.x + 0.5));\n"
            "  }\n"
            "\n"
            ...
            "  );\n"
            "  edges = clamp(edges + 0.5 * softEdges, vec4(0.5 / float(N)), vec4(1.0 - 0.5 / float(N)));\n"
            "  if (pattern == 4) {\n"
