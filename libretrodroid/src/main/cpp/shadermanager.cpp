...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "}\n"
    "\n"
    "Pattern computePattern(Pixels pixels, lowp vec4 edgeWeights, bool triangle, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "\n"
    ...
    "  }\n"
    "\n"
    "  Pattern pattern = computePattern(pixels, flags.edgeWeight, flags.triangle, pxCoords);\n"
    "\n"
    "  lowp vec3 final = blend(\n"
    ...
    const std::string ShaderManager::cut3UpscalePass0Fragment =
            ...
            "  else if (agc.x + agc.y < EDGE_MIN_VALUE) { pattern = 0; }\n"
            "  else if (sign(gc.x * gc.y) > 0.0) { pattern = 2; }\n"
            "  else { pattern = 1; }\n"
            "  return pattern;\n"
            "}\n"
            "\n"
            "lowp float reduceAAWeight(lowp float a, lowp float b, lowp float c, lowp float d) {\n"
            "  lowp vec4 pxs = vec4(a, b, c, d);\n"
            "  pxs = normalize(pxs + vec4(EPSILON));\n"
            "  lowp float result = 0.0;\n"
            "  result += clamp(abs(pxs.y - pxs.w) - max(abs(pxs.y - pxs.z), abs(pxs.z - pxs.w)), 0.0, 1.0);\n"
            "  result -= clamp(abs(pxs.x - pxs.z) - max(abs(pxs.x - pxs.y), abs(pxs.y - pxs.z)), 0.0, 1.0);\n"
            "  return clamp(2.0 * result, -1.0, 1.0);\n"
            "}\n"
//            "lowp float reduceAAWeight(lowp float x, lowp float y, lowp float z, lowp float w) {\n"
//            "  lowp vec4 n = normalize(vec4(x, y, z, w) + vec4(EPSILON));\n"
//            "  lowp float result = 0.0;\n"
//            "  result += abs(n.x - n.z) * abs(mix(n.x, n.z, 0.5) - n.y);\n"
//            "  result -= abs(n.y - n.w) * abs(mix(n.y, n.w, 0.5) - n.z);\n"
//            "  return clamp(2.0 * result, -1.0, 1.0);\n"
//            "}\n"
//            "lowp float reduceAAWeight(lowp float a, lowp float b, lowp float c, lowp float d) {\n"
//            "  lowp vec4 pxs = vec4(a, b, c, d);\n"
//            "  pxs = normalize(pxs + vec4(EPSILON));\n"
//            "  lowp float result = 0.0;\n"
//            "  result += abs(pxs.y - pxs.w) * min(abs(pxs.y - pxs.z), abs(pxs.z - pxs.w));\n"
//            "  result -= abs(pxs.x - pxs.z) * min(abs(pxs.x - pxs.y), abs(pxs.y - pxs.z));\n"
//            "  return step(0.125, result) - step(0.125, -result);\n"
//            "}\n"
//            "lowp float reduceAAWeight(lowp float a, lowp float b, lowp float c, lowp float d) {\n"
//            "  lowp vec4 ws = vec4(abs(d - b), abs(c - d), abs(a - c), abs(b - a));\n"
//            "  ws = normalize(ws + vec4(EPSILON));\n"
//            "  lowp float result = min(ws.x, ws.y) - min(ws.z, ws.w);\n"
//            "  return clamp(2.0 * result, -1.0, 1.0);\n"
//            "}\n"
            "\n"
            "void main() {\n"
            ...
            "  lowp vec4 ec = abs(vec4(l06, l10, l09, l05) - vec4(l05, l06, l10, l09));\n"
            "\n"
            "  int originalPattern = accuratePattern(gn, ge, gs, gw, gc, ec);\n"
            "  int pattern = originalPattern;\n"
            "\n"
            "  bool isTriangle = pattern >= 3;\n"
            "  bool isInvertedTriangle = pattern == 4;\n"
            "\n"
            "  ivec4 ns = ivec4(\n"
            "    fastPattern(gn),\n"
            "    fastPattern(ge),\n"
            ...
            "  );\n"
            "\n"
            "  bool vertical = ns.x == 1 || ns.z == 1;\n"
            "  bool horizontal = ns.y == 2 || ns.w == 2;\n"
            "  bvec4 opposite = equal(ns, ivec4(pattern == 3 ? 4 : 3));\n"
            "  if (isTriangle && ((vertical && any(opposite.yw)) || (horizontal && any(opposite.xz)))) {\n"
            "    pattern = 0;\n"
            "  }\n"
            "\n"
            "  lowp vec4 result = vec4(0.0);\n"
            "  result.x = float(originalPattern) / 4.0;\n"
            "  result.y = float(pattern) / 4.0;\n"
            "\n"
            "#if REDUCE_ANTI_ALIASING\n"
            "  lowp vec4 edges = vec4(\n"
            "    reduceAAWeight(l04, l05, l06, l07),\n"
            "    reduceAAWeight(l02, l06, l10, l14),\n"
            "    reduceAAWeight(l08, l09, l10, l11),\n"
            "    reduceAAWeight(l01, l05, l09, l13)\n"
            "  );\n"
            "  result.z = quickPackFloats2(edges.xy * 0.5 + vec2(0.5));\n"
            "  result.w = quickPackFloats2(edges.zw * 0.5 + vec2(0.5));\n"
            "#endif\n"
            "\n"
            "  gl_FragColor = result;\n"
            "}";
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "}\n"
            "\n"
            "int fetchPattern(lowp float value) {\n"
            ...
            "    int currentPattern = fetchPattern(texture2D(previousPass, coords).y);\n"
            "    if (currentPattern == 3) { result.y = results.x; }\n"
            "    else if (currentPattern == 4) { result.y = results.y; }\n"
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += HSTEP; }\n"
            "    else if (currentPattern == continuePattern) { result.x += STEP; }\n"
            ...
            "  int pattern = fetchPattern(previousPassPixel.y);\n"
            "\n"
            ...
            "\n"
            "#if REDUCE_ANTI_ALIASING\n"
            "  lowp vec4 softEdges = (2.0 * REDUCE_ANTI_ALIASING_AMOUNT - EPSILON) * vec4(\n"
            "    quickUnpackFloats2(previousPassPixel.z + 0.001953125) - vec2(0.5),\n"
            "    quickUnpackFloats2(previousPassPixel.w + 0.001953125) - vec2(0.5)\n"
            "  );\n"
            "\n"
            "  edges += (vec4(1.0) - abs(edges)) * softEdges;\n"
            "#endif\n"
            "\n"
            "  int originalPattern = fetchPattern(previousPassPixel.x);\n"
...
//            "  gl_FragColor = vec4(color, 1.0);\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Vertex =
            ...
            "void main() {\n"
            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001;\n"
//            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.20 + vec2(0.1, 0.4);\n"
            "  screenCoords = coords * textureSize - vec2(0.5);\n"
            "  c05 = (screenCoords + vec2(+0.0, +0.0)) / textureSize;\n"
            "  c06 = (screenCoords + vec2(+1.0, +0.0)) / textureSize;\n"
            "  c09 = (screenCoords + vec2(+0.0, +1.0)) / textureSize;\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Fragment =
            ...
            "\n"
            "  gl_FragColor = vec4(final.rgb, 1.0);\n"
//            "  edges = abs(edges - vec4(0.5));\n"
//            "  gl_FragColor = vec4(vec3(max(max(edges.x, edges.y), max(edges.z, edges.w))), 1.0);\n"
            "}";
