...
const std::string ShaderManager::cut2UpscaleFragment =
    ...
    "  if (p.x < edge0) {\n"
    "    lowp float l1 = sharpSmooth(0.0, 1.0, 1.0 - p.x / tana - p.y, sharpness);\n"
    "    lowp float l2 = min(1.0 - l1, sharpSmooth(0.0, 1.0, p.y, sharpness));\n"
    "    lowp float l3 = 1.0 - l1 - l2;\n"
    "    w = vec4(l1 + l3 * (1.0 - tana), l3 * tana, l2, 0.0);\n"
    "  } else if (p.x > edge1) {\n"
    "    lowp float l1 = sharpSmooth(0.0, 1.0, (p.x - 1.0 + p.y * tana) / tana, sharpness);\n"
    "    lowp float l2 = min(1.0 - l1, sharpSmooth(0.0, 1.0, 1.0 - p.y, sharpness));\n"
    "    lowp float l3 = 1.0 - l2 - l1;\n"
    "    w = vec4(0.0, l2, l3 * tana, l1 + l3 * (1.0 - tana));\n"
    "  } else {\n"
