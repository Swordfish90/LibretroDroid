...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "  lowp float diagonal = 0.5 + tana * (0.5 - p.y);\n"
    "  lowp vec4 ws = vec4(0.0);\n"
    "  if (tana < 0.1) {\n"
    "    lowp float x = sharpSmooth(0.0, 1.0, p.x, sharpness);\n"
    "    lowp float y = sharpSmooth(0.0, 1.0, p.y, sharpness);\n"
    "    ws = vec4((1.0 - x) * (1.0 - y), x * (1.0 - y), (1.0 - x) * y, x * y);\n"
    "  } else {\n"
    "    lowp float l3r = sharpSmooth(0.0, 1.0, (1.0 - p.y), sharpness);\n"
    "    lowp float l1r = min(1.0 - l3r, sharpSmooth(0.0, 1.0, p.x + tana * (p.y - 1.0), sharpness));\n"
    "    lowp float l2r = 1.0 - l1r - l3r;\n"
    "\n"
    "    lowp float l3l = sharpSmooth(0.0, 1.0, p.y, sharpness);\n"
    "    lowp float l1l = min(1.0 - l3l, sharpSmooth(0.0, 1.0, 1.0 - p.x - p.y * tana, sharpness));\n"
    "    lowp float l2l = 1.0 - l1l - l3l;\n"
    "    ws = mix(vec4(l1l, l2l, l3l, 0.0), vec4(0.0, l3r, l2r, l1r), step(diagonal, p.x));\n"
    "  }\n"
    "  return a * ws.x + b * ws.y + c * ws.z + d * ws.w;\n"
    "}\n"
    "\n"
