...
//            "}\n"
            ...
            "  lowp vec2 gw = abs(vec2(l04 - l09, l05 - l08));\n"
            "  lowp vec2 gc = abs(vec2(l05 - l10, l06 - l09));\n"
            "  lowp vec2 agc = vec2(l05 - l10, l06 - l09);\n"
            "\n"
            "  lowp vec4 ec = abs(vec4(l06, l10, l09, l05) - vec4(l05, l06, l10, l09));\n"
            "  lowp vec2 cg = 0.25 * (gn + ge + gs + gw);\n"
            "\n"
            ...
            "  bool leftTriangle = 2.0 * gc.y + cg.y + EPSILON < gc.x + cg.x && allowRightTriangle;\n"
            "\n"
            // TODO FILIPPO... Find rejections to be put in the alpha channel...
            // TODO FILIPPO... Extract this somewhere else...
            "  lowp float pattern = 0.0;\n"
            "  if (leftTriangle) { pattern = 1.00; }\n"
            "  else if (rightTriangle) { pattern = 0.75; }\n"
            "  else if (gc.x + gc.y < 0.10) { pattern = 0.00; }\n"
            "  else if (sign(agc.x * agc.y) > 0.0) { pattern = 0.50; }\n"
            "  else { pattern = 0.25; }\n"
            "\n"
            "  bvec2 flags = bvec2(leftTriangle || rightTriangle, leftTriangle);\n"
            "\n"
            ...
            "  gl_FragColor = vec4(\n"
            "    pattern,\n"
            "    quickPackFloats2(edges.xy),\n"
            "    quickPackFloats2(edges.zw),\n"
            "    1.0\n"
            "  );\n"
            "}";
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "    else if (currentPattern == 4) { result.y = 0.0; }\n"
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += HSTEP; }\n"
            "    else if (currentPattern == 1) { result.x += STEP; }\n"
            "    if (currentPattern != 1) { break; }\n"
            "  }\n"
            ...
            "  return t < 0.5 ? mix(a, 0.5, t * 2.0) : mix(0.5, b, (t - 0.5) * 2.0);\n"
            "}\n"
            "lowp float blend(lowp vec2 d1, lowp vec2 d2) {\n"
            "  lowp float result = 0.5;\n"
            "  if (d1.x + d2.x <= float(N) * STEP) {\n"
            "    result = blend(d1.y, d2.y, d1.x / (d1.x + d2.x));\n"
            "  } else if (d1.x <= M) {\n"
            "    result = mix(d1.y, 0.5, d1.x / M);\n"
            "  } else if (d2.x <= M) {\n"
            "    result = mix(d2.y, 0.5, d2.x / M);\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "void main() {\n"
            ...
            "  lowp vec4 edges = vec4(0.5);\n"
            "\n"
            "  lowp vec2 resultN = vec2(0.0, 0.5);\n"
            "  lowp vec2 resultS = vec2(0.0, 0.5);\n"
            "  lowp vec2 resultW = vec2(0.0, 0.5);\n"
            "  lowp vec2 resultE = vec2(0.0, 0.5);\n"
            "\n"
            ...
            "    edges.z = blend(vec2(HSTEP, 0.0), resultS);\n"
            "    edges.w = blend(resultW, vec2(HSTEP, 1.0));\n"
            "    edges.y = blend(vec2(HSTEP, 0.0), resultE);\n"
            "  } else if (pattern == 4) {\n"
            "    edges.x = blend(resultN, vec2(HSTEP, 0.0));\n"
            "    edges.z = blend(vec2(HSTEP, 1.0), resultS);\n"
            "    edges.w = blend(resultW, vec2(HSTEP, 0.0));\n"
...
//            "  gl_FragColor = vec4(color, 1.0);\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Vertex =
            ...
            "void main() {\n"
            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001;\n"
//            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.25 + vec2(0.1);\n"
            "  screenCoords = coords * textureSize - vec2(0.5);\n"
            "  c05 = (screenCoords + vec2(+0.0, +0.0)) / textureSize;\n"
            "  c06 = (screenCoords + vec2(+1.0, +0.0)) / textureSize;\n"
            "  c09 = (screenCoords + vec2(+0.0, +1.0)) / textureSize;\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Fragment =
            ...
            "  lowp float displacement = 2.0 * (midPoint - 0.5);\n"
