...
const std::unordered_map<std::string, std::string> ShaderManager::cut3UpscaleParams = {
    ...
    { "SOFT_EDGES_SHARPENING", "1" },
    { "SOFT_EDGES_SHARPENING_AMOUNT", "0.75" },
    { "SEARCH_MIN_CONTRAST", "0.5" },
    { "SEARCH_MAX_DISTANCE", "4" },
};
...
const std::string ShaderManager::cut3UpscalePass0Fragment =
    ...
    "  }\n"
    "\n"
    "  if (localContrast < SEARCH_MIN_CONTRAST * globalContrast) {\n"
    "    result = -result;\n"
    "  }\n"
    "\n"
    "  return result;\n"
    ...
    "}\n"
    "\n"
    "bool isPatternNotContinuous(lowp int pattern, lowp ivec4 neighborPatterns, lowp float contrast, lowp vec4 neighborContrasts) {\n"
    "  bool result = false;\n"
    "\n"
    "  if (pattern == 1) {\n"
    "    result = neighborPatterns.x == 1 && contrast < SEARCH_MIN_CONTRAST * neighborContrasts.x ||\n"
    "      (neighborPatterns.z == 1 && contrast < SEARCH_MIN_CONTRAST * neighborContrasts.z);\n"
    "  } else if (pattern == 2) {\n"
    "    result = neighborPatterns.y == 2 && contrast < SEARCH_MIN_CONTRAST * neighborContrasts.y ||\n"
    "      (neighborPatterns.w == 2 && contrast < SEARCH_MIN_CONTRAST * neighborContrasts.w);\n"
    "  }\n"
    "\n"
    "  return result;\n"
    "}\n"
    ...
    "    findPattern(quads[1], quads[0].localContrast),\n"
    "    findPattern(quads[2], quads[0].localContrast),\n"
    "    findPattern(quads[3], quads[0].localContrast),\n"
    "    findPattern(quads[4], quads[0].localContrast)\n"
    "  );\n"
    "\n"
    "  lowp int pattern = findPattern(quads, 0.0);\n"
    "\n"
    "  lowp vec4 neighborContrasts = vec4(\n"
    "    quads[1].localContrast,\n"
    "    quads[2].localContrast,\n"
    "    quads[3].localContrast,\n"
    "    quads[4].localContrast\n"
    "  );\n"
    "\n"
    ...
    "\n"
    "  bool reject = isTriangle && (any(opposite) || (vertical && horizontal));\n"
    "  reject = reject || isPatternNotContinuous(pattern, neighbors, quads[0].localContrast, neighborContrasts);\n"
    "\n"
    "  if (reject) {\n"
    "    pattern = -pattern;\n"
    "  }\n"
