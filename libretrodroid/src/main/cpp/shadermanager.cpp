...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "\n"
            "#if REDUCE_ANTI_ALIASING\n"
            "  lowp vec4 softEdges = 2.0 * REDUCE_ANTI_ALIASING_AMOUNT * vec4(\n"
...
//            "  lowp vec4 edgeMixWeights = clamp(5.0 * (abs(softEdges)), 0.0, 1.0);\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Fragment =
            ...
            "\n"
            "  lowp vec3 final = blend(\n"
            "    blend(pattern.pixels.p0, pattern.pixels.p1, pattern.weights.x, pattern.midPoints.x, 1.0),\n"
            "    blend(pattern.pixels.p2, pattern.pixels.p3, pattern.weights.y, pattern.midPoints.y, 1.0),\n"
            "    pattern.weights.z,\n"
            "    pattern.midPoints.z,\n"
            "    float(!flags.triangle)\n"
            "  );\n"
            "\n"
            "  gl_FragColor = vec4(final.rgb, 1.0);\n"
            "}";
