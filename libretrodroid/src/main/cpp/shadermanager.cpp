...
//    "  threshold += softEdge * EDGE_SHARPENING_AMOUNT * mix(1.0, 0.5, distance(luma(a), luma(b)));\n"
//    "  threshold += softEdge * EDGE_SHARPENING_AMOUNT;\n"
//    "  threshold += min(EDGE_SHARPENING_AMOUNT, 1.0 - distance(luma(a), luma(b))) * softEdge;\n"
//    "  threshold += clamp(0.5 - abs(0.5 - distance(luma(a), luma(b))), 0.0, 1.0) * softEdge;\n"
//    "  threshold += (1.0 - 2.0 * abs(0.5 - distance(luma(a), luma(b)))) * softEdge * EDGE_SHARPENING_AMOUNT;\n"
//    "  lowp float attenuation = clamp(3.0 - 4.0 * distance(luma(a), luma(b)), 0.0, 1.0);\n"
    "  lowp float contrast = distance(luma(a), luma(b));\n"
    "  lowp float attenuation = (1.0 - contrast * contrast);\n"
    "  threshold += attenuation * EDGE_SHARPENING_AMOUNT * softEdge;\n"
    "#endif\n"
    "  lowp float sharpness = clamp(sharpness(luma(a), luma(b)), 0.0, 0.5 * (1.0 - abs(threshold)));\n"
