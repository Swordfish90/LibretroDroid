...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "}\n"
    "\n"
    "int findPattern(lowp vec4 values, lowp vec2 diagonals) {\n"
    ...
    "  );\n"
    "\n"
    "  patternContrasts.zw += diagonals.yx - diagonals.xy;\n"
    "\n"
    ...
    "}\n"
    "\n"
    "lowp float hardEdgeWeight(int cp, int np, int vertical, int positiveDiagonal, int negativeDiagonal) {\n"
    "  lowp float result = 0.0;\n"
    ...
    "\n"
    "  int cp = findPattern(vec4(l05, l06, l09, l10), vec2(0.0));\n"
    "  ivec4 ns = ivec4(\n"
    ...
    "    hardEdgeWeight(cp, ns.x, 1, 4, 3),\n"
    "    hardEdgeWeight(cp, ns.y, 2, 3, 4),\n"
    "    hardEdgeWeight(cp, ns.z, 1, 3, 4),\n"
    "    hardEdgeWeight(cp, ns.w, 2, 4, 3)\n"
    "  );\n"
    "\n"
...
const std::string ShaderManager::cut3UpscalePass1Fragment =
    ...
    "    int currentPattern = fetchPattern(texture2D(previousPass, coords).x);\n"
    "\n"
    "    if (currentPattern == 3) {\n"
    "      result.y = results.x;\n"
    ...
    "#endif\n"
    "\n"
    "  int originalPattern = pattern;\n"
    "  if (originalPattern < 0) { originalPattern = -originalPattern; }\n"
