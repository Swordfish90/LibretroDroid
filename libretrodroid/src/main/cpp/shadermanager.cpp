...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "  lowp float dx = min(sharpness, dist * 0.5);\n"
    ...
    "  lowp float edgeStrength = 0.5 * (absEdgeDirection.x + absEdgeDirection.y);\n"
    "\n"
...
//    "  lowp float contrast = (lmax - lmin) / (lmax + lmin + 0.05);\n"
    ...
    "  bool invertedAngle = hasAngle && absEdgeDirection.y > absEdgeDirection.x;\n"
    "  bool negativeAngle = hasAngle && (edgeDirection.x * edgeDirection.y) < 0.0;\n"
    "\n"
    "  lowp float tannum = min(absEdgeDirection.x, absEdgeDirection.y);\n"
    "  lowp float tanden = max(absEdgeDirection.x, absEdgeDirection.y);\n"
