...
const std::unordered_map<std::string, std::string> ShaderManager::cut2UpscaleParams = {
    ...
    { "EDGE_SHARPENING_MIN_CONTRAST_EDGE", "0.1" },
    { "EDGE_SHARPENING_MAX_CONTRAST_EDGE", "0.1" },
...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "}\n"
    "\n"
//    "lowp float softEdgeWeight(lowp vec4 left, lowp vec4 right) {\n"
//    "  lowp float maxRange = max(maxOf(left), maxOf(right));\n"
//    "  lowp float minRange = min(minOf(left), minOf(right));\n"
//    "  lowp float threshold = max(EDGE_SHARPENING_MIN_VALUE, EDGE_SHARPENING_THRESHOLD * (maxRange - minRange));\n"
//    "  return step(threshold, abs(left.x - right.w)) - step(threshold, abs(right.x - left.w));\n"
//    "}\n"
    "\n"
    "lowp float softEdgeWeight(lowp float l0, lowp float l1, lowp float l2, lowp float l3) {\n"
    "  lowp float maxRange = maxOf(l0, l1, l2, l3);\n"
    "  lowp float minRange = minOf(l0, l1, l2, l3);\n"
    "  lowp float threshold = max(EDGE_SHARPENING_MIN_CONTRAST_EDGE, EDGE_SHARPENING_THRESHOLD * (maxRange - minRange));\n"
    "  return step(threshold, abs(l3 - l1)) * (1.0 - step(threshold, abs(l2 - l1))) * (1.0 - step(threshold, abs(l2 - l3))) - step(threshold, abs(l2 - l0)) * (1.0 - step(threshold, abs(l0 - l1))) * (1.0 - step(threshold, abs(l2 - l1)));\n"
    "}\n"
    "\n"
    "void main() {\n"
    ...
    "//    softEdgeWeight(vec4(l05, l01, l09, l04), vec4(l06, l02, l10, l07)),\n"
    "//    softEdgeWeight(vec4(l06, l07, l05, l02), vec4(l10, l11, l09, l14)),\n"
    "//    softEdgeWeight(vec4(l09, l05, l13, l08), vec4(l10, l06, l14, l11)),\n"
    "//    softEdgeWeight(vec4(l05, l06, l04, l01), vec4(l09, l10, l08, l13))\n"
    "  );\n"
    "\n"
...
const std::string ShaderManager::cut2UpscalePass1Vertex =
    ...
    "void main() {\n"
    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001;\n"
//    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.25 + vec2(0.0, 0.0);\n"
//    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.25 + vec2(0.20, 0.0);\n"
//    "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.25 + vec2(0.25, 0.25);\n"
    "  screenCoords = coords * textureSize - vec2(0.5);\n"
    "  c05 = (screenCoords + vec2(+0.0, +0.0)) / textureSize;\n"
    "  c06 = (screenCoords + vec2(+1.0, +0.0)) / textureSize;\n"
    "  c09 = (screenCoords + vec2(+0.0, +1.0)) / textureSize;\n"
...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "}\n"
    "\n"
    "lowp float triangle(lowp float edge0, lowp float edge1, lowp float t) {\n"
    "  lowp float center = mix(edge0, edge1, 0.5);\n"
    "  lowp float width = 0.5 * (edge1 - edge0);\n"
    "  return clamp(1.0 - abs(t - center) / width, 0.0, 1.0);\n"
    "}\n"
    "\n"
    "lowp float sharpness(lowp float l1, lowp float l2) {\n"
    "#if USE_DYNAMIC_BLEND\n"
    ...
    "  lowp float smoothness = 0.5 - sharpness(luma(a), luma(b));\n"
    "#if EDGE_SHARPENING\n"
//    "  lowp float attenuation = clamp(3.0 - 4.0 * distance(luma(a), luma(b)), 0.0, 1.0);\n"
//    "  lowp float contrast = distance(luma(a), luma(b));\n"
//    "  lowp float attenuation = triangle(EDGE_SHARPENING_MIN_CONTRAST_EDGE, EDGE_SHARPENING_MAX_CONTRAST_EDGE, contrast);\n"
//    "  lowp float attenuation = 1.0 - contrast;\n"
    "  displacement += 1.0 * EDGE_SHARPENING_AMOUNT * softEdge;\n"
    "#endif\n"
//    "  sharpness = min(sharpness, 0.5 * (1.0 - abs(threshold)));\n"
//    "  sharpness *= step(abs(threshold), 0.05);\n"
//    "  sharpness = mix(sharpness, 0.0, 2.0 * abs(threshold));\n"
//    "  sharpness = max(sharpness - 0.5 * abs(threshold), 0.0);\n"
    "  lowp float centre = 0.5 + displacement;\n"
    "  smoothness = min(smoothness, min(1.0 - centre, centre));\n"
...
//    "  final = mix(final, vec3(flags.edgeWeight.x * 0.5 + 0.5), (step(0.25, pxCoords.x) - step(0.75, pxCoords.x)) * (step(0.00, pxCoords.y) - step(0.25, pxCoords.y)));\n"
//    "  final = mix(final, vec3(flags.edgeWeight.y * 0.5 + 0.5), (step(0.25, pxCoords.y) - step(0.75, pxCoords.y)) * (step(0.75, pxCoords.x) - step(1.00, pxCoords.x)));\n"
//    "  final = mix(final, vec3(flags.edgeWeight.z * 0.5 + 0.5), (step(0.25, pxCoords.x) - step(0.75, pxCoords.x)) * (step(0.75, pxCoords.y) - step(1.00, pxCoords.y)));\n"
//    "  final = mix(final, vec3(flags.edgeWeight.w * 0.5 + 0.5), (step(0.25, pxCoords.y) - step(0.75, pxCoords.y)) * (step(0.00, pxCoords.x) - step(0.25, pxCoords.x)));\n"
    "\n"
