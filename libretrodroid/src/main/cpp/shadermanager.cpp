...
const std::string ShaderManager::cut3UpscalePass0Fragment =
    ...
    "};\n"
    "\n"
    "Block readBlock(lowp vec4 values) {\n"
    ...
    "}\n"
    "\n"
    "lowp int findPattern(Block block, lowp float globalContrast) {\n"
    "  return computePattern(block.scores, block.localContrast, globalContrast);\n"
    "}\n"
    "\n"
    "lowp int findPattern(Block blocks[5], lowp float globalContrast) {\n"
    "  lowp vec4 scores = blocks[0].scores;\n"
    "  lowp mat4 adjustments = mat4(\n"
    "    relativeScores(blocks[1].scores),\n"
    "    relativeScores(blocks[2].scores),\n"
    "    relativeScores(blocks[3].scores),\n"
    "    relativeScores(blocks[4].scores)\n"
    "  );\n"
    "  scores += 0.125 * (adjustments[0] + adjustments[1] + adjustments[2] + adjustments[3]);\n"
    "  return computePattern(scores, blocks[0].localContrast, globalContrast);\n"
    "}\n"
    "\n"
    "lowp float softEdgeWeight(lowp float a, lowp float b, lowp float c, lowp float d) {\n"
    "  lowp float result = 0.0;\n"
    ...
    "\n"
    "  Block blocks[5];\n"
    "  blocks[0] = readBlock(vec4(l05, l06, l09, l10));\n"
    "  blocks[1] = readBlock(vec4(l01, l02, l05, l06));\n"
    "  blocks[2] = readBlock(vec4(l06, l07, l10, l11));\n"
    "  blocks[3] = readBlock(vec4(l09, l10, l13, l14));\n"
    "  blocks[4] = readBlock(vec4(l04, l05, l08, l09));\n"
    "\n"
    "  lowp int pattern = findPattern(blocks, globalContrast);\n"
    "\n"
    "  lowp ivec4 neighbors = ivec4(\n"
    "    findPattern(blocks[1], globalContrast),\n"
    "    findPattern(blocks[2], globalContrast),\n"
    "    findPattern(blocks[3], globalContrast),\n"
    "    findPattern(blocks[4], globalContrast)\n"
    "  );\n"
    "\n"
