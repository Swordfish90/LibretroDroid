...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "  lowp vec4 ds = abs(values.ywzx - values.xywz);\n"
    "  lowp float contrast = max(max(ds.x, ds.y), max(ds.z, ds.w));\n"
    "\n"
    "  bool isDiagonal = max(max(ds.x + ds.y, ds.z + ds.w), max(ds.x + ds.w, ds.y + ds.z)) >= max(ds.x + ds.z, ds.y + ds.w);\n"
    "\n"
    "  lowp vec2 crossGradient = vec2(values.x - values.w, values.y - values.z);\n"
    "  lowp vec2 absCrossGradient = abs(crossGradient);\n"
    "  lowp vec2 absNormalGradient = abs(vec2(crossGradient.x - crossGradient.y, crossGradient.x + crossGradient.y));\n"
    ...
    "    result = 0;\n"
    "  } else if (!isDiagonal) {\n"
    "    if (absNormalGradient.x > absNormalGradient.y + EPSILON) {\n"
    "      result = 1;\n"
    "    } else if (absNormalGradient.y > absNormalGradient.x + EPSILON) {\n"
    "      result = 2;\n"
    "    }\n"
    "  } else {\n"
    ...
    "  }\n"
    "\n"
    "  lowp float minCrossGradient = min(absCrossGradient.x, absCrossGradient.y);\n"
    "  lowp float maxCrossGradients = max(absCrossGradient.x, absCrossGradient.y);\n"
    "\n"
    "  if (minCrossGradient > 0.25 * maxCrossGradients + EPSILON && minCrossGradient + EPSILON < 0.75 * maxCrossGradients) {\n"
    "    result = -result;\n"
    "  }\n"
    "\n"
    "  return result;\n"
...
const std::string ShaderManager::cut3UpscalePass0Fragment =
    ...
    "  bool reject = isTriangle && (vertical && any(opposite.yw) || horizontal && any(opposite.xz));\n"
    "  if (reject) {\n"
    "    pattern = -pattern;\n"
    "  }\n"
    "\n"
