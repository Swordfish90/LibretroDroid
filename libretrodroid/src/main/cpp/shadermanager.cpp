...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "  }\n"
            "\n"
            "  lowp vec4 softEdges = vec4(0.5) + vec4(0.5) * vec4(\n"
            "    quickUnpackFloats2(previousPassPixel.y + 0.001953125) - vec2(0.5),\n"
            "    quickUnpackFloats2(previousPassPixel.z + 0.001953125) - vec2(0.5)\n"
            "  );\n"
            "\n"
            "  edges.x = abs(edges.x - 0.5) >= abs(softEdges.x - 0.5) ? edges.x : softEdges.x;\n"
            "  edges.y = abs(edges.y - 0.5) >= abs(softEdges.y - 0.5) ? edges.y : softEdges.y;\n"
            "  edges.z = abs(edges.z - 0.5) >= abs(softEdges.z - 0.5) ? edges.z : softEdges.z;\n"
            "  edges.w = abs(edges.w - 0.5) >= abs(softEdges.w - 0.5) ? edges.w : softEdges.w;\n"
            "\n"
            "  if (pattern == 4) {\n"
