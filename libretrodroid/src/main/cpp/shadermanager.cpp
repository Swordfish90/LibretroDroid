...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    const std::string ShaderManager::cut3UpscalePass0Fragment =
            ...
            "lowp float quickPackFloats2(lowp vec2 values) {\n"
            "  return dot(\n"
            "    floor(values * vec2(PACKING_PRECISION_SCALE) + vec2(0.5)),\n"
            "    vec2(1.0 / (PACKING_PRECISION_BITS), 1.0 / (PACKING_PRECISION_BITS * PACKING_PRECISION_BITS))\n"
            "  );\n"
            "}\n"
            "\n"
            "lowp float minOf(lowp float a, lowp float b, lowp float c) {\n"
            "  return min(a, min(b, c));\n"
            "}\n"
            "int fastPattern(lowp vec2 gc) {\n"
            "  lowp vec2 agc = abs(gc);\n"
            "  int pattern = 0;\n"
            "  if (agc.x + agc.y < 0.10) { pattern = 0; }\n"
            "  else if (agc.x > 2.0 * agc.y) { pattern = 4; }\n"
            "  else if (agc.y > 2.0 * agc.x) { pattern = 3; }\n"
            "  else if (sign(gc.x * gc.y) > 0.0) { pattern = 2; }\n"
            "  else { pattern = 1; }\n"
            "  return pattern;\n"
            "}\n"
            "int accuratePattern(lowp vec2 gn, lowp vec2 ge, lowp vec2 gs, lowp vec2 gw, lowp vec2 gc, lowp vec4 ec) {\n"
            "  lowp vec2 agc = abs(gc);\n"
            "  lowp vec2 cg = 0.25 * (abs(gn) + abs(ge) + abs(gs) + abs(gw));\n"
            "\n"
            "  lowp float oppositeEdgesContrast = max(ec.x + ec.z, ec.y + ec.w) - EPSILON;\n"
            "  bool allowLeftTriangle = max(ec.w + ec.z, ec.x + ec.y) >= oppositeEdgesContrast;\n"
            "  bool allowRightTriangle = max(ec.w + ec.x, ec.z + ec.y) >= oppositeEdgesContrast;\n"
            "  bool rightTriangle = 2.0 * agc.x + cg.x + EPSILON < agc.y + cg.y && allowLeftTriangle;\n"
            "  bool leftTriangle = 2.0 * agc.y + cg.y + EPSILON < agc.x + cg.x && allowRightTriangle;\n"
            "\n"
            "  int pattern = 0;\n"
            "  if (leftTriangle) { pattern = 4; }\n"
            "  else if (rightTriangle) { pattern = 3; }\n"
            "  else if (agc.x + agc.y < 0.10) { pattern = 0; }\n"
            "  else if (sign(gc.x * gc.y) > 0.0) { pattern = 2; }\n"
            "  else { pattern = 1; }\n"
            "  return pattern;\n"
            "}\n"
            "\n"
//            "lowp float softEdgeWeight(lowp float a, lowp float b, lowp float c, lowp float d) {\n"
//            "  lowp vec4 pxs = vec4(a, b, c, d);\n"
//            "  pxs = normalize(pxs + vec4(EPSILON));\n"
//            "  lowp float result = 0.0;\n"
//            "  result += min(min(abs(pxs.y - pxs.z), abs(pxs.y - pxs.w)), abs(pxs.z - pxs.w));\n"
//            "  result -= min(min(abs(pxs.x - pxs.y), abs(pxs.y - pxs.z)), abs(pxs.x - pxs.z));\n"
...
//            "}\n"
            ...
            "  lowp float l14 = luma(t14);\n"
            "\n"
            "  lowp vec2 gn = vec2(l01 - l06, l02 - l05);\n"
            "  lowp vec2 ge = vec2(l06 - l11, l07 - l10);\n"
            "  lowp vec2 gs = vec2(l09 - l14, l10 - l13);\n"
            "  lowp vec2 gw = vec2(l04 - l09, l05 - l08);\n"
            "  lowp vec2 gc = vec2(l05 - l10, l06 - l09);\n"
            "\n"
            "  lowp vec4 ec = abs(vec4(l06, l10, l09, l05) - vec4(l05, l06, l10, l09));\n"
            "\n"
            "  int pattern = accuratePattern(gn, ge, gs, gw, gc, ec);\n"
            "\n"
            ...
            "  );\n"
//            "  edges = vec4(0.0);\n"
//            "  edges = sign(edges) * clamp(floor(abs(edges) * 4.0 + 0.5) / 4.0 + 0.5 / 4.0, -1.0, 1.0);\n"
//            "  edges = clamp(floor((edges * 0.5 + vec4(0.5)) * 4.0 + 0.5) / 4.0 + (0.5 / 4.0), 0.0, 1.0);\n"
            "\n"
            "  bvec2 flags = bvec2(pattern >= 3, pattern == 4);\n"
            "\n"
            "  ivec4 ns = ivec4(\n"
            "    fastPattern(gn),\n"
            "    fastPattern(ge),\n"
            ...
            "    fastPattern(gw)\n"
            "  );\n"
            "  if (pattern == 3 && (((ns.x == 1 || ns.z == 1) && (ns.y == 4 || ns.w == 4)) || ((ns.y == 2 || ns.w == 2) && (ns.x == 4 || ns.z == 4)))) { pattern = 0; }\n"
            "  if (pattern == 4 && (((ns.x == 1 || ns.z == 1) && (ns.y == 3 || ns.w == 3)) || ((ns.y == 2 || ns.w == 2) && (ns.x == 3 || ns.z == 3)))) { pattern = 0; }\n"
            "\n"
            "  gl_FragColor = vec4(\n"
            "    float(pattern) / 4.0,\n"
            "    quickPackFloats2(edges.xy * 0.5 + vec2(0.5)),\n"
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "  lowp float current = value;\n"
            "\n"
            "  current *= PACKING_PRECISION_BITS;\n"
            "  result.x = floor(current);\n"
            "  current -= result.x;\n"
            "\n"
            "  current *= PACKING_PRECISION_BITS;\n"
            ...
            "  current -= result.y;\n"
            "\n"
            "  return result / PACKING_PRECISION_SCALE;\n"
            "}\n"
            "\n"
            "bvec2 quickUnpackBools2(lowp float value) {\n"
            "  lowp vec2 result = vec2(0.0);\n"
            ...
            "}\n"
            "\n"
            // TODO FILIPPO... Rename into pattern...
            "int roundToInt(mediump float value) {\n"
            "  return int(value * 4.0 + 0.5);\n"
            "}\n"
            // TODO FILIPPO... Rename them N and M are not really descriptive...
            "#define N 4\n"
            "const float STEP = 0.5 / float(N);\n"
            "const float HSTEP = (STEP * 0.5);\n"
            "const float M = STEP * float(N / 2) + HSTEP;\n"
            "\n"
            "lowp vec2 walkN(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  lowp vec2 result = vec2(0.0, 0.0);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(0.0, -dc.y * float(i));\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            "    if (currentPattern == 3) { result.y = -1.0; }\n"
            "    else if (currentPattern == 4) { result.y = 1.0; }\n"
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += HSTEP; }\n"
            "    else if (currentPattern == 1) { result.x += STEP; }\n"
            "    if (currentPattern != 1) { break; }\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "lowp vec2 walkS(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  lowp vec2 result = vec2(0.0, 0.0);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(0.0, +dc.y * float(i));\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            ...
            "  return result;\n"
            "}\n"
            "lowp vec2 walkE(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  lowp vec2 result = vec2(0.0, 0.0);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(dc.x * float(i), 0.0);\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            "    if (currentPattern == 3) { result.y = 1.0; }\n"
            "    else if (currentPattern == 4) { result.y = -1.0; }\n"
            "    if (currentPattern == 3 || currentPattern == 4) { result.x += HSTEP; }\n"
            "    else if (currentPattern == 2) { result.x += STEP; }\n"
            "    if (currentPattern != 2) { break; }\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "lowp vec2 walkW(lowp sampler2D previousPass, HIGHP vec2 baseCoords) {\n"
            "  lowp vec2 result = vec2(0.0, 0.0);\n"
            "  for (int i = 1; i <= N; i++) {\n"
            "    HIGHP vec2 coords = baseCoords + vec2(-dc.x * float(i), 0.0);\n"
            "    int currentPattern = roundToInt(texture2D(previousPass, coords).x);\n"
            ...
            "}\n"
            "\n"
            "lowp float blend(lowp float a, lowp float b, lowp float t) {\n"
            "  return t < 0.5 ? mix(a, 0.0, t * 2.0) : mix(0.0, b, (t - 0.5) * 2.0);\n"
            "}\n"
            "lowp float blend(lowp vec2 d1, lowp vec2 d2) {\n"
            "  lowp float result = 0.0;\n"
            "  if (d1.x + d2.x <= float(N) * STEP) {\n"
            "    result = blend(d1.y, d2.y, d1.x / (d1.x + d2.x));\n"
            "  } else if (d1.x <= M) {\n"
            "    result = mix(d1.y, 0.0, d1.x / M);\n"
            "  } else if (d2.x <= M) {\n"
            "    result = mix(d2.y, 0.0, d2.x / M);\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "void main() {\n"
            "  lowp vec4 previousPassPixel = texture2D(previousPass, passCoords);\n"
            "  int pattern = roundToInt(previousPassPixel.x);\n"
            "\n"
            ...
            "    resultN = walkN(previousPass, passCoords);\n"
            "    resultS = walkS(previousPass, passCoords);\n"
            "  }\n"
            "  if (pattern == 2 || pattern == 3 || pattern == 4) {\n"
            "    resultW = walkW(previousPass, passCoords);\n"
            "    resultE = walkE(previousPass, passCoords);\n"
            "  }\n"
            "\n"
            "  if (pattern == 1) {\n"
            "    edges.x = blend(resultN, resultS + vec2(STEP, 0.0));\n"
            "    edges.z = blend(resultN + vec2(STEP, 0.0), resultS);\n"
            "  } else if (pattern == 2) {\n"
            "    edges.w = blend(resultW, resultE + vec2(STEP, 0.0));\n"
            "    edges.y = blend(resultW + vec2(STEP, 0.0), resultE);\n"
            "  } else if (pattern == 3) {\n"
            "    edges.x = blend(resultN, vec2(HSTEP, 1.0));\n"
            "    edges.z = blend(vec2(HSTEP, -1.0), resultS);\n"
            "    edges.w = blend(resultW, vec2(HSTEP, 1.0));\n"
            "    edges.y = blend(vec2(HSTEP, -1.0), resultE);\n"
            "  } else if (pattern == 4) {\n"
            "    edges.x = blend(resultN, vec2(HSTEP, -1.0));\n"
            "    edges.z = blend(vec2(HSTEP, 1.0), resultS);\n"
            "    edges.w = blend(resultW, vec2(HSTEP, -1.0));\n"
...
//            "  edges += min(vec4(1.0 - edges), edges) * softEdges;\n"
            "  edges = clamp(edges + softEdges, -1.0, 1.0);\n"
//            "  edges += min(vec4(1.0 - edges), edges) * softEdges;\n"
//            "  edges = clamp(edges, vec4(0.5 / float(N)), vec4(1.0 - 0.5 / float(N)));\n"
            "\n"
            "  gl_FragColor = vec4(\n"
            "    previousPassPixel.w,\n"
            "    quickPackFloats2(edges.xy * 0.5 + vec2(0.5)),\n"
            ...
            "  );\n"
//            "  gl_FragColor = vec4(\n"
//            "    abs(softEdges.xyz - 0.5),\n"
//            "    1.0\n"
//            "  );\n"
//            "  lowp vec3 color = vec3(0.0);\n"
//            "  if (pattern == 0) color = vec3(0.0, 0.0, 0.0);\n"
//            "  if (pattern == 1) color = vec3(1.0, 0.0, 0.0);\n"
//            "  if (pattern == 2) color = vec3(0.0, 1.0, 0.0);\n"
//            "  if (pattern == 3) color = vec3(1.0, 1.0, 0.0);\n"
//            "  if (pattern == 4) color = vec3(0.0, 1.0, 1.0);\n"
//            "  gl_FragColor = vec4(color, 1.0);\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Vertex =
            ...
            "void main() {\n"
            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001;\n"
//            "  HIGHP vec2 coords = vec2(vCoordinate.x, mix(vCoordinate.y, 1.0 - vCoordinate.y, vFlipY)) * 1.0001 * 0.25 + vec2(0.0);\n"
            "  screenCoords = coords * textureSize - vec2(0.5);\n"
            "  c05 = (screenCoords + vec2(+0.0, +0.0)) / textureSize;\n"
            "  c06 = (screenCoords + vec2(+1.0, +0.0)) / textureSize;\n"
            "  c09 = (screenCoords + vec2(+0.0, +1.0)) / textureSize;\n"
    ...
    const std::string ShaderManager::cut3UpscalePass2Fragment =
            ...
            "  return 0.5 * result;\n"
            "}\n"
            "lowp vec3 blend(lowp vec3 a, lowp vec3 b, lowp float t, lowp float midPoint) {\n"
            "  lowp float sharpness = sharpness(luma(a), luma(b));\n"
            ...
            "  bool secondTriangle = triangle && pxCoords.x >= 1.0 - (1.0 - pxCoords.y);\n"
            "\n"
            "  ShapeWeights shapeWeights = triangle\n"
            "    ? triangleWeights(firstTriangle ? pxCoords : pxCoords.yx, firstTriangle ? edgeWeights.wz : edgeWeights.xy)\n"
            "    : quadWeights(pxCoords, edgeWeights);\n"
            "\n"
            ...
            "\n"
            "  lowp vec3 final = blend(\n"
            "    blend(pattern.pixels.p0, pattern.pixels.p1, pattern.weights.x, pattern.midPoints.x),\n"
            "    blend(pattern.pixels.p2, pattern.pixels.p3, pattern.weights.y, pattern.midPoints.y),\n"
            "    pattern.weights.z,\n"
            "    pattern.midPoints.z\n"
            "  );\n"
            "\n"
            "  gl_FragColor = vec4(final.rgb, 1.0);\n"
//            "  gl_FragColor = vec4(mix(vec3(edges.w), vec3(edges.y), step(0.5, pxCoords.x)), 1.0);\n"
            "}";
