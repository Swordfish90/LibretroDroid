...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "  return dot(vec3(float(a), float(b), float(c)), vec3(0.52, 0.26, 0.13));\n"
    "}\n"
    "\n"
    "void main() {\n"
...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "}\n"
    "\n"
    "bvec3 unpack(lowp float value) {\n"
    "  lowp vec3 values = fract(value * vec3(1.0, 2.0, 4.0));\n"
    "  return greaterThan(values, vec3(0.5));\n"
    "}\n"
    "\n"
    ...
    "  bvec3 transform = unpack(flagsTexture.y);\n"
    "  bvec3 patternFlags = unpack(flagsTexture.z);\n"
    "\n"
    "  lowp vec2 pxCoords = fract(screenCoords);\n"
    "\n"
    "  if (transform.x) {\n"
    ...
    "  }\n"
    "\n"
    "  lowp vec3 ab = patternFlags.y ? pixels.p0 : pixels.p1;\n"
    "  lowp vec3 cd = patternFlags.x ? pixels.p2 : pixels.p3;\n"
    "\n"
    "  Pattern pattern;\n"
    "\n"
    "  if (patternType == 0) {\n"
