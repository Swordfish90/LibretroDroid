...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    const std::string ShaderManager::cut3UpscalePass0Fragment =
            ...
            "  else if (gc.x + gc.y < 0.20) { pattern = 0.00; }\n"
    ...
    const std::string ShaderManager::cut3UpscalePass1Fragment =
            ...
            "  if (d1.x + d2.x + STEP < float(M * 2) * STEP) {\n"
            "    result = blend(d1.y, d2.y, d1.x / (d1.x + d2.x));\n"
            "  } else if (d1.x < float(M) * STEP) {\n"
            "    result = mix(d1.y, 0.5, d1.x / (float(M) * STEP));\n"
            "  } else if (d2.x < float(M) * STEP) {\n"
            "    result = mix(d2.y, 0.5, d2.x / (float(M) * STEP));\n"
            "  }\n"
            "  return result;\n"
            "}\n"
            "void main() {\n"
            ...
            "    edges.z = blend(resultS, vec2(HSTEP, 0.0));\n"
            "    edges.w = blend(resultW, vec2(HSTEP, 1.0));\n"
            "    edges.y = blend(resultE, vec2(HSTEP, 0.0));\n"
            "  } else if (pattern == 4) {\n"
            "    edges.x = blend(resultN, vec2(HSTEP, 0.0));\n"
            "    edges.z = blend(resultS, vec2(HSTEP, 1.0));\n"
            "    edges.w = blend(resultW, vec2(HSTEP, 0.0));\n"
            "    edges.y = blend(resultE, vec2(HSTEP, 1.0));\n"
            "  }\n"
            "\n"
            ...
            "  );\n"
            "  edges = clamp(edges + 0.5 * softEdges, 0.05, 0.95);\n"
            "  if (pattern == 4) {\n"
