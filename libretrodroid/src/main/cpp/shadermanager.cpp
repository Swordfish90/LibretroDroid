...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "  lowp float val1 = floor(value * 3.0);\n"
    "  lowp float val2 = floor((value * 3.0 - val1) * 3.0);\n"
    "  return clamp(vec2(val1 / 2.0, val2 / 2.0), vec2(0.0), vec2(1.0));\n"
    "}\n"
    "\n"
    ...
    "#endif\n"
    "  lowp float sharpness = max(sharpness(luma(a), luma(b)), 0.5 * abs(threshold));\n"
    ...
    "    result.softEdgeWeights = vec3(edgeWeights.w, 0.0, 0.0);\n"
    "  } else {\n"
    ...
    "    result.edgeWeights = vec3(0.0, edgeWeights.z, 0.0);\n"
    "    result.softEdgeWeights = vec3(edgeWeights.x, 0.0, mix(0.0, edgeWeights.y, pxCoords.x));\n"
    "  }\n"
    "  return result;\n"
    "}\n"
    "\n"
    ...
    "    result.weights.z = pxCoords.x - 0.5 * pxCoords.y;\n"
    "    result.edgeWeights = vec3(edgeWeights.x, 0.0, 0.0);\n"
    "    result.softEdgeWeights = vec3(0.0, edgeWeights.y, 0.0);\n"
    "  } else {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p1, pixels.p2, pixels.p3);\n"
    "    result.weights.x = pxCoords.x - 0.5 * pxCoords.y;\n"
    ...
    "    result.softEdgeWeights = vec3(0.0, 0.0, 0.0);\n"
    "  }\n"
    "  return result;\n"
    "}\n"
    "\n"
