...
const std::string ShaderManager::cut2UpscaleFragment =
    ...
    "  lowp float tana,\n"
    "  lowp vec2 p,\n"
    "  lowp float sharpnessHigh,\n"
    "  lowp float sharpnessLow\n"
    ") {\n"
    ...
    "  return blend(blend(p0, p1, x0, sharpnessHigh), blend(p2, p3, x1, sharpnessHigh), p.y, sharpnessLow);\n"
    "}\n"
    "\n"
    "void main() {\n"
    ...
    "\n"
    "  bool hasEdge = edgeStrength > MIN_EDGE;\n"
    "  bool invertedAngle = hasEdge && absTanDirection.y > absTanDirection.x;\n"
    "  bool negativeAngle = hasEdge && (tanDirection.x * tanDirection.y) >= 0.0;\n"
    "\n"
    "  lowp float tannum = min(absTanDirection.x, absTanDirection.y);\n"
    "  lowp float tanden = max(absTanDirection.x, absTanDirection.y);\n"
    "  lowp float atana = hasEdge ? (tannum / (tanden + 0.01)) : 0.0;\n"
    "\n"
