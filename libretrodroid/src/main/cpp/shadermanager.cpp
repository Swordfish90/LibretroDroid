...
const std::string ShaderManager::cut2UpscalePass0Fragment =
    ...
    "}\n"
    "bvec2 toBools(lowp float value) {\n"
    "  return bvec2(value > 0.5, mod(value, 0.5) > 0.25);\n"
    "}\n"
    "\n"
    "void main() {\n"
    ...
    "\n"
    "  if (any(type5)) {\n"
    "    flags.type = bvec3(true, false, true);\n"
    "    flags.flip = bvec3(type5.z, type5.x, type5.y);\n"
    "    flags.cuts = bvec2(false, false);\n"
    "  } else if (any(type4)) {\n"
    "    flags.type = bvec3(false, false, true);\n"
    "    flags.flip = bvec3(type4.w, type4.y, type4.x || type4.y);\n"
    "    flags.cuts = bvec2(bottomCut, topCut);\n"
    "  } else if (any(type3)) {\n"
    "    flags.type = bvec3(true, true, false);\n"
    "    flags.flip = bvec3(type3.w, type3.y, type3.x || type3.y);\n"
    "    flags.cuts = bvec2(bottomCut, !topCut);\n"
    "  } else if (any(type2_v)) {\n"
    "    flags.type = bvec3(false, true, false);\n"
    "    flags.flip = bvec3(type2_v.x || type2_v.w, type2_v.y || type2_v.x, false);\n"
    "    flags.cuts = bvec2(bottomCut || topCut, false);\n"
    "  } else if (any(type2_h)) {\n"
    "    flags.type = bvec3(false, true, false);\n"
    "    flags.flip = bvec3(type2_h.y || type2_h.x, type2_h.w || type2_h.x, true);\n"
    "    flags.cuts = bvec2(bottomCut || topCut, false);\n"
    "  } else if (any(type1)) {\n"
    "    flags.type = bvec3(true, false, false);\n"
    "    flags.flip = bvec3(type1.y, false, false);\n"
    "    flags.cuts = bvec2(false, false);\n"
    "  }\n"
    "\n"
    "#if REDUCE_ANTIALIASING\n"
    "  lowp vec4 laplacian = vec4(0.0);\n"
    "  bvec2 b05 = toBools(1.0 - max(abs(l04 - l06), abs(l01 - l09)));\n"
    "  bvec2 b06 = toBools(1.0 - max(abs(l05 - l07), abs(l02 - l10)));\n"
    "  bvec2 b09 = toBools(1.0 - max(abs(l08 - l10), abs(l05 - l13)));\n"
    "  bvec2 b10 = toBools(1.0 - max(abs(l09 - l11), abs(l06 - l14)));\n"
    "  flags.gradients = bvec4(b05.x, b06.x, b09.x, b10.x);\n"
    "  flags.gradients2 = bvec4(b05.y, b06.y, b09.y, b10.y);\n"
    "#endif\n"
    "\n"
    "  gl_FragColor = vec4(\n"
    "    pack(flags.type.x, flags.type.y, flags.type.z, flags.cuts.x),\n"
    "    pack(flags.flip.x, flags.flip.y, flags.flip.z, flags.cuts.y),\n"
    "    pack(flags.gradients.x, flags.gradients.y, flags.gradients.z, flags.gradients.w),\n"
    "    pack(flags.gradients2.x, flags.gradients2.y, flags.gradients2.z, flags.gradients2.w)\n"
    "  );\n"
    "}";
...
const std::string ShaderManager::cut2UpscalePass1Fragment =
    ...
    "};\n"
    "\n"
    "lowp vec4 floatUnpack(lowp float values) {\n"
    "  return vec4(\n"
    "    floor(mod(values / 2.0, 2.0)),\n"
    "    floor(mod(values / 4.0, 2.0)),\n"
    "    floor(mod(values / 8.0, 2.0)),\n"
    "    floor(mod(values / 16.0, 2.0))\n"
    "  );\n"
    "}\n"
    "\n"
    "bvec4 boolUnpack(lowp float values) {\n"
    "  return greaterThan(floatUnpack(values), vec4(0.5));\n"
    "}\n"
    "Flags parseFlags(lowp vec4 flagsPixel) {\n"
    "  bvec4[2] flagBits;\n"
    "  flagBits[0] = boolUnpack(floor(flagsPixel.x * 255.0 + 0.5));\n"
    "  flagBits[1] = boolUnpack(floor(flagsPixel.y * 255.0 + 0.5));\n"
    "\n"
    "  ivec3 typeBits = ivec3(flagBits[0].rgb);\n"
    "\n"
    "  Flags flags;\n"
    "  flags.type = typeBits.x + 2 * typeBits.y + 4 * typeBits.z;\n"
    "  flags.flip = flagBits[1].rgb;\n"
    "  flags.cuts = bvec2(flagBits[0].w, flagBits[1].w);\n"
    "\n"
    "#if REDUCE_ANTIALIASING\n"
    "  flags.gradients = vec4(0.0);\n"
    "  flags.gradients += 0.66 * floatUnpack(floor(flagsPixel.z * 255.0 + 0.5));\n"
    "  flags.gradients += 0.33 * floatUnpack(floor(flagsPixel.w * 255.0 + 0.5));\n"
    "#endif\n"
    "\n"
    "  return flags;\n"
    "}\n"
    ...
    "  lowp float nt = t + REDUCE_ANTIALIASING_AMOUNT * (b.a - a.a) * (0.5 - sharpness);\n"
    "  return Pixel(mix(a.rgb, b.rgb, sharpSmooth(nt, sharpness)), mix(a.a, b.a, t));\n"
    "#else\n"
    "  return mix(a, b, sharpSmooth(t, sharpness(luma(a), luma(b))));\n"
    "#endif\n"
    "}\n"
    "\n"
    "Pattern pattern0(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  return Pattern(pixels, false, pxCoords);\n"
    "}\n"
    "\n"
    "Pattern pattern1(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  if (pxCoords.y > pxCoords.x) {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p2, pixels.p2, pixels.p3);\n"
    "    result.triangle = true;\n"
    ...
    "}\n"
    "\n"
    "Pattern pattern2(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    ...
    "}\n"
    "\n"
    "Pattern pattern3(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    "  if (pxCoords.y > 2.0 * pxCoords.x) {\n"
    "    result.pixels = Pixels(pixels.p0, pixels.p2, pixels.p2, cd);\n"
    "    result.triangle = true;\n"
    ...
    "}\n"
    "\n"
    "Pattern pattern4(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    ...
    "}\n"
    "\n"
    "Pattern pattern5(Pixels pixels, lowp Pixel ab, lowp Pixel cd, lowp vec2 pxCoords) {\n"
    "  Pattern result;\n"
    ...
    "    vec4(t05, flags.gradients.x),\n"
    "    vec4(t06, flags.gradients.y),\n"
    "    vec4(t09, flags.gradients.z),\n"
    "    vec4(t10, flags.gradients.w)\n"
    "  );\n"
    "#else\n"
    "  Pixels pixels = Pixels(t05, t06, t09, t10);\n"
    "#endif\n"
    ...
    "  }\n"
    "\n"
    "  lowp Pixel ab = flags.cuts.y ? pixels.p0 : pixels.p1;\n"
    "  lowp Pixel cd = flags.cuts.x ? pixels.p2 : pixels.p3;\n"
    "\n"
    "  Pattern pattern;\n"
    "\n"
    "  if (flags.type == 0) {\n"
    ...
    "  lowp vec3 weights = pattern.triangle ? triangle(pattern.coords) : quad(pattern.coords);\n"
    "\n"
    "  lowp Pixel final = blend(\n"
    "    blend(pattern.pixels.p0, pattern.pixels.p1, weights.x),\n"
    "    blend(pattern.pixels.p2, pattern.pixels.p3, weights.y),\n"
    "    weights.z\n"
    "  );\n"
    "  pxCoords = fract(screenCoords);\n"
//    "  if (pxCoords.x < 0.5 && pxCoords.y < 0.5) { final = vec4(vec3(1.0 - flags.gradients.x), 1.0); }\n"
//    "  if (pxCoords.x > 0.5 && pxCoords.y < 0.5) { final = vec4(vec3(1.0 - flags.gradients.y), 1.0); }\n"
//    "  if (pxCoords.x < 0.5 && pxCoords.y > 0.5) { final = vec4(vec3(1.0 - flags.gradients.z), 1.0); }\n"
//    "  if (pxCoords.x > 0.5 && pxCoords.y > 0.5) { final = vec4(vec3(1.0 - flags.gradients.w), 1.0); }\n"
    "\n"
