...
const std::string ShaderManager::cut2UpscaleFragment =
    ...
    "  return mix(edge0, edge1, smoothstep(edge0 + dx, edge1 - dx, t));\n"
    "}\n"
    "lowp float ss(lowp float t, lowp float sharpness) {\n"
    "  return sharpSmooth(0.0, 1.0, t, sharpness);\n"
    "}\n"
    "\n"
    ...
    ") {\n"
    "  lowp float edge0 = tana * (1.0 - p.y);\n"
    "  lowp float edge1 = 1.0 - tana * p.y;\n"
    "  lowp vec2 ws = vec2(0.0);\n"
    "  lowp vec3 points[4];\n"
    "  if (p.x < edge0) {\n"
    "    ws = vec2(p.x + p.y * tana, p.y + p.x / tana);\n"
    "    points[0] = a; points[1] = b; points[2] = a; points[3] = c;\n"
    "  } else if (p.x > edge1) {\n"
    "    ws = vec2(p.y - (1.0 - p.x) / tana, p.x - (1.0 - p.y) * tana);\n"
    "    points[0] = b; points[1] = d; points[2] = c; points[3] = d;\n"
    "  } else {\n"
    "    ws = vec2(p.x + tana * p.y, p.x - (1.0 - p.y) * tana);\n"
    "    points[0] = a; points[1] = b; points[2] = c; points[3] = d;\n"
    "  }\n"
    "  lowp vec3 p0 = mix(points[0], points[1], ss(ws.x, sharpnessHigh));\n"
    "  lowp vec3 p1 = mix(points[2], points[3], ss(ws.y, sharpnessHigh));\n"
    "  return mix(p0, p1, ss(p.y, sharpnessLow));\n"
    "}\n"
    "\n"
    "void main() {\n"
    ...
    "  lowp float sharpnessHigh = max(SHARPNESS_MIN, contrast * SHARPNESS_MAX);\n"
    "  lowp float sharpnessLow = SHARPNESS_MIN;\n"
    "#else\n"
    "  lowp float sharpnessHigh = SHARPNESS_MAX;\n"
    "  lowp float sharpnessLow = SHARPNESS_MIN;\n"
    "#endif\n"
    "\n"
    "  lowp vec2 pxCoords = fract(screenCoords);\n"
    "\n"
    "  bool hasAngle = edgeStrength > MIN_EDGE;\n"
    ...
    "    sharpnessLow\n"
    "  );\n"
    "  final = mix(final, vec3(1.0, 0.0, 0.0), step(0.95, coords.x) * step(0.95, coords.y));\n"
    "\n"
...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "  final = mix(final, vec3(0.0, 0.0, 1.0), step(0.95, coords.x) * step(0.95, coords.y));\n"
    "\n"
    "  gl_FragColor = vec4(final, 1.0);\n"
    "}\n";
