...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "  } else if (p.x >= diagonal) {\n"
//    "    lowp float l1 = tana * (-1.0 + p.x / tana + p.y);\n"
//    "    lowp float l2 = 1.0 - p.x;\n"
//    "    lowp float l3 = tana * (1.0 - p.y);\n"
    "    lowp float l3 = sharpSmooth(0.0, 1.0, (1.0 - p.y), sharpness);\n"
    "    lowp float l1 = min(1.0 - l3, sharpSmooth(0.0, 1.0, p.x + tana * (p.y - 1.0), sharpness));\n"
    "    lowp float l2 = 1.0 - l1 - l3;\n"
    "    return d * l1 + c * l2 + b * l3;\n"
//    "    return vec3(0.0);\n"
    "  } else {\n"
//    "    lowp float l1 = 1.0 - p.x - p.y * tana;\n"
//    "    lowp float l2 = p.x;\n"
//    "    lowp float l3 = p.y * tana;\n"
//    "    return a * l1 + b * l2 + mix(a, c, 1.0 / tana) * l3;\n"
//    "    p.x = sharpSmooth(0.0, 1.0, p.x, sharpness);\n"
    "    lowp float l3 = sharpSmooth(0.0, 1.0, p.y, sharpness);\n"
    "    lowp float l1 = min(1.0 - l3, sharpSmooth(0.0, 1.0, 1.0 - p.x - p.y * tana, sharpness));\n"
    "    lowp float l2 = 1.0 - l1 - l3;//p.y * tana;\n"
    "    return a * l1 + b * l2 + c * l3;\n"
//    "    return vec3(l3);\n"
    "  }\n"
    "}\n"
//    "lowp vec3 interpolate(lowp vec3 a, lowp vec3 b, lowp vec3 c, lowp vec3 d, lowp float tana, lowp vec2 p, lowp float sharpness) {\n"
//    "  lowp vec3 s = mix(a, b, tana);\n"
//    "  lowp vec3 g = mix(c, d, 1.0 - tana);\n"
//    "  lowp float diagonal = 0.5 + tana * (0.5 - p.y);\n"
//    "  lowp vec3 gg = mix(c, d, -tana);\n"
////    "  lowp vec3 gg = a + g - b;\n"
//    "  lowp vec3 ss = mix(a, b, 1.0 + tana);\n"
////    "  lowp vec3 ss = s + d - c;\n"
//    "  if (p.x >= diagonal) {\n"
//    "    lowp float xu = sharpSmooth(tana, 1.0 + tana, p.x + p.y * tana, sharpness);\n"
//    "    lowp float xd = sharpSmooth(0.0, 1.0, p.x - (1.0 - p.y) * tana, sharpness);\n"
//    "    return mix(mix(s, ss, xu), mix(c, d, xd), sharpSmooth(0.0, 1.0, p.y, sharpness));\n"
//    "  } else {\n"
//    "    lowp float xu = sharpSmooth(0.0, 1.0, p.x + p.y * tana, sharpness);\n"
//    "    lowp float xd = sharpSmooth(-tana, 1.0 - tana, p.x - (1.0 - p.y) * tana, sharpness);\n"
//    "    return mix(mix(a, b, xu), mix(gg, g, xd), sharpSmooth(0.0, 1.0, p.y, sharpness));\n"
//    "  }\n"
//    "}\n"
//    // Poly Net
//    "lowp vec3 interpolate(lowp vec3 a, lowp vec3 b, lowp vec3 c, lowp vec3 d, lowp float tana, lowp vec2 p, lowp float sharpness) {\n"
//    "  lowp vec3 fab = mix(a, b, p.x);\n"
//    "  lowp vec3 fbd = mix(b, d, p.y);\n"
//    "  lowp vec3 fdc = mix(d, c, 1.0 - p.x);\n"
//    "  lowp vec3 fca = mix(c, a, 1.0 - p.y);\n"
//    "  mediump vec3 result = fab * (1.0 - p.y) + fbd * p.x + fdc * p.y + fca * (1.0 - p.x);\n"
//    "  result -= (1.0 - p.x) * (1.0 - p.y) * a + p.x * (1.0 - p.y) * b + p.x * p.y * c + (1.0 - p.x) * (p.y) * d;\n"
//    "  return result;\n"
//    "}\n"
//    "lowp vec3 interpolate(lowp vec3 a, lowp vec3 b, lowp vec3 c, lowp vec3 d, lowp float tana, lowp vec2 p, lowp float sharpness) {\n"
...
//    "  lowp vec3 t4 = vec3(1.0, 1.0, 0.0);\n"
//    "  lowp vec3 t3 = vec3(0.0, 1.0, 0.0);\n"
//    "  lowp vec3 t2 = vec3(1.0, 0.0, 1.0);\n"
//    "  lowp vec3 t1 = vec3(0.0, 0.0, 1.0);\n"
