...
const std::string ShaderManager::cut3UpscalePass1Fragment =
    ...
    "  lowp float l10 = luma(t10);\n"
    "\n"
    "  // TODO FILIPPO... Move this to previous step... \n"
    "  lowp float lmax = max(max(l5, l6), max(l9, l10));\n"
    "  lowp float lmin = min(min(l5, l6), min(l9, l10));\n"
    "  lowp float contrast = (lmax - lmin) / (lmax + lmin + 0.05);\n"
    "  lowp float sharpness = 0.5 * linearStep(SHARPNESS_MIN, SHARPNESS_MAX, contrast);\n"
    "\n"
    "  lowp vec2 pxCoords = fract(screenCoords);\n"
    "\n"
    ...
    "  if (triangulate) {\n"
    "    final = triangleInterpolate(t5, t6, t9, t10, flags, pxCoords, sharpness);\n"
    "  } else {\n"
    "    final = quadInterpolate(t5, t6, t9, t10, flags, pxCoords, sharpness);\n"
    "  }\n"
    "\n"
    "  gl_FragColor = vec4(final, 1.0);\n"
    "}";
