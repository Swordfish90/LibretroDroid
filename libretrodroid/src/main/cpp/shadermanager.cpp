...
const std::string ShaderManager::cutUpscaleFragment =
    ...
    "#endif\n"
    "\n"
    "  lowp vec2 d = vec2(l1 - l2 + l4 - l3, l1 + l2 - l4 - l3);\n"
    "\n"
    "  lowp vec2 pxCoords = fract(screenCoords);\n"
    "\n"
    "  bool hasAngle = abs(d.x) + abs(d.y) >= 0.05;\n"
    "  bool invertAngle = hasAngle && abs(d.y) > abs(d.x);\n"
    "  lowp float tana = invertAngle ? d.x / d.y : d.y / d.x;\n"
    "\n"
